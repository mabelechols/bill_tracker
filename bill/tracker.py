...
def track(in_dir, out_dir, cat_file):
    ...
    with open(os.path.join(out_dir, "all_transactions.csv"), "w", newline="") as f_stream:
    ...
    if len(uncat) > 0:
        ...
        print(f"Uncategorized vendors written to {os.path.join(out_dir, "uncategorized.txt")}")